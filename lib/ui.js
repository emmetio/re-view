	...
	var featureFactory = {
		syncScroll: function(enabled) {
			var ui = panelOption(
				...
				<label for="emr-sync-scroll">Sync scrolling</label>'
			);

			var checkbox = ui.querySelector('input[type="checkbox"]');
			checkbox.checked = !!enabled;
			checkbox.addEventListener('change', function(evt) {
				event.trigger('option:sync-scroll', checkbox.checked);
			}, false);
			return ui;
		},
		maxWidth: function(params) {
			var ui = panelOption(
				...
				<i class="emmet-re-view__icon" data-icon="checkbox"></i>\
				<label for="emr-max-width-enabled">Max Viewport Width</label>\
			...
			checkbox.addEventListener('change', function(evt) {
				event.trigger('option:max-width-enabled', checkbox.checked);
			}, false);

			input.addEventListener('input', function(evt) {
				event.trigger('option:max-width', validate());
			});
	...
	return {
		...
		create: function(features) {
			...
			if (features.panel) {
				...
				['maxWidth', 'syncScroll'].forEach(function(feat) {
					if (feat in features) {
